...
public class MobRSA {
	...
	public BigInteger[] genKeys() throws Throwable {
		...
		return genKeys(BigInteger.probablePrime((this.keySize / 2) - 1, secureRandom),
				BigInteger.probablePrime((this.keySize / 2) - 1, secureRandom),
				BigInteger.probablePrime(this.keySize / 2, secureRandom));
	}
	...
	public BigInteger[] genKeys(BigInteger bigInteger, BigInteger bigInteger2, BigInteger bigInteger3)
		...
		bigInteger = multiply.subtract(bigInteger).subtract(bigInteger2).add(BigInteger.ONE);
		if (bigInteger3.compareTo(bigInteger) >= 0) {
			throw new Throwable("e must be smaller than (p-1)*(q-1)");
		} else if (bigInteger.gcd(bigInteger3).compareTo(BigInteger.ONE) != 0) {
			throw new Throwable("e must be coprime with (p-1)*(q-1)");
		} else {
			bigInteger = bigInteger3.modInverse(bigInteger);
	...
	public byte[] encode(byte[] bArr, BigInteger bigInteger, BigInteger bigInteger2) throws Throwable {
		...
		while (bArr2.length > i3) {
			...
			byte[] encodeBlock = encodeBlock(bArr2, i3, min, bigInteger, bigInteger2, i);
